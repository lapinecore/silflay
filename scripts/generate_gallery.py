...
def build_gallery_markdown(image_files, thumbnails_dir: Path, images_dir: Path):
    if not image_files:
    ...
    for image_path in image_files:
        ...
        lines.append(f'| [![{alt}]({thumb_rel})]({image_rel}) | [{image_path.name}]({image_rel}) |')
...
def main():
    ...
    gallery_markdown = build_gallery_markdown(image_files, thumbnails_dir, images_dir)
